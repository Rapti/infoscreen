//
// Created by leon on 26.07.18.
//

#ifndef INFOSCREEN_GAUSSIANBLUR_H
#define INFOSCREEN_GAUSSIANBLUR_H
...
//      constants for high performance Gaussian blurRadius filter implementation (separable,
...
inline std::string GenerateGaussFunctionCode( int kernelSize, bool workaroundForNoCLikeArrayInitialization = false )
{
	...
	shaderCode += eol;
	shaderCode += "uniform sampler2D texture;" + eol;
	shaderCode += "uniform float size;" + eol;
	shaderCode += "uniform vec2 dir;" + eol;
...
//	shaderCode += "vec4 GaussianBlur( sampler2D tex0, vec2 centreUV, vec2 halfPixelOffset, vec2 pixelOffset )                                                                           " + eol;
	shaderCode += "void main() {                                                                           " + eol;
	shaderCode += "    vec4 colOut = vec4(0, 0, 0, 0);                                                                                                                                  " + eol;
	shaderCode += "    vec2 centreUV = gl_TexCoord[0].xy;                                                                                                                                  " + eol;
	shaderCode += "    vec2 pixelOffset = dir / size;                                                                                                                                  " + eol;
	shaderCode += "                                                                                                                                                                     " + eol;
	...
	shaderCode += "        vec4 col = texture2D( texture, centreUV + texCoordOffset ) + texture2D( texture, centreUV - texCoordOffset );                                                " + eol;
	shaderCode += "        colOut += gWeights[i] * col;                                                                                                                               " + eol;
	shaderCode += "    }                                                                                                                                                                " + eol;
	shaderCode += "                                                                                                                                                                     " + eol;
	shaderCode += "    gl_FragColor = colOut;                                                                                                                                                   " + eol;
	shaderCode += "}                                                                                                                                                                    " + eol;
//	shaderCode += "void main() {gl_FragColor = GaussianBlur(texture, gl_TexCoord[0].xy, 0, dir / size);}";
	shaderCode += eol;

	return shaderCode;
}

#endif //INFOSCREEN_GAUSSIANBLUR_H
